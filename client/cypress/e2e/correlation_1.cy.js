describe('Correlation', () => {
  ...
  const relationSheetColumn = '/sizes/size@code_external';
  const numberOfExpectedMatches = 3971;
  ...
  it('Correlation', () => {
    ...
    cy.get('.loadFiles__input').first().selectFile('./cypress/e2e/files/comarch_cennik_zrzut.csv', {
      action: 'drag-drop'
    });
    cy.get('.loadFiles__input').last().selectFile('./cypress/e2e/files/idosell_zrzut_towarow.csv', {
      action: 'drag-drop'
    });
    ...
    cy.get('.input--matchThreshold').clear().type('100')
